...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, decompose_type, cr, num_classes=1000, channel=3):
        ...
        self.conv1_custom = nn.Conv2d(channel, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)
...
def resnet101(decompose_type, cr, pretrained=False, channel= 20, **kwargs):

    model = ResNet(Bottleneck, [3, 4, 23, 3], decompose_type, cr, num_classes=101, channel=channel, **kwargs)
    if pretrained:
        if decompose_type == None:
            pretrain_dict = model_zoo.load_url(model_urls['resnet101'])
        else:
            module_pretrain_dict = torch.load('./ResNet/outputs/ResNet101/' + decompose_type + '/cr_' + str(cr) + '/model_best.pth.tar')['state_dict']
            pretrain_dict = OrderedDict()
            for k, v in module_pretrain_dict.items():
...
if __name__ == '__main__':
    model = resnet101(decompose_type='RCP', cr=0.1, pretrained=True, channel=20)

...
class PathInfo(object):
    ...
    def __init__(self, contraction_list, input_subscripts, output_subscript, indices, path, scale_list, naive_cost,
        ...
        self.naive_cost = float(naive_cost)
        ...
        self.largest_intermediate = float(max(size_list))
...
def contract_path(*operands, **kwargs):
    ...
    *operands : list of array_like
    ...
    # Python side parsing
    if '|' in operands[0]: #Check if a convolution is occurring
        conv_subscripts=operands[0].split('|')[1] #Obtain indices to convolve over.
        conv_subscripts=conv_subscripts.split(",")
        pure_operands=[i for i in operands] #Prepare operand for parsing.
        pure_operands[0]=operands[0].split('|')[0] #Insert clean einsum string. 
        einoutput=operands[0].split("->")[1] #Original output.
        input_subscripts, output_subscript, operands = parser.parse_einsum_input(pure_operands)
    else:
        conv_subscripts=""
    ...
    for tnum, term in enumerate(input_list):
        ...
        for cnum, char in enumerate(term):
            ...
            if char in size_dict and (char not in conv_subscripts):
                # For broadcasting cases we always want the largest dim size
                #tr: weird handling that seems superfluous. 
                if size_dict[char] == 1:
                    size_dict[char] = dim
            ...
            elif char not in size_dict and char in conv_subscripts:
                size_dict[char]={}
                size_dict[char][tnum]=dim #Convolution leg is a list of dimensions in the dictionary.
                size_dict[char]["max"]=dim
            elif char in size_dict and char in conv_subscripts:
                size_dict[char][tnum]=dim #List of different dimensions for a convolving leg.
                convdims=[size_dict[char][pos] for pos in size_dict[char]]
                size_dict[char]["max"]=max(convdims) #Update the max dim for this conv subscript. 
            else:
                size_dict[char] = dim #Non-conv leg not already in the dictionary.   

    #Dictionary of intermediate products
    intprods={}
    for pos,inputs in enumerate(input_list):
        intprods["".join(sorted(inputs))]=[pos] #Index the base tensors by its shape.
        intprods[pos]=set(inputs) #Also index the base shapes by its base tensor.  
    
    # Compute size of each input array plus the output array
    size_list_input = [helpers.compute_size_by_dict(term, size_dict, conv_subscripts,intprods,"".join(sorted(term))) for term in input_list]
    size_list_output = [helpers.compute_size_by_dict(term, size_dict, conv_subscripts,intprods,"".join(sorted(term))) for term in output_subscript]
    size_list = size_list_input + size_list_output
    ...
    # tr: Yes, this cost computation is quite strange...

    inner_product = (sum(len(x) for x in input_sets) - len(indices)) > 0 #what..?
    ...
    for cnum, contract_inds in enumerate(path):
        ...
        contract_tuple = helpers.find_contraction(contract_inds, input_sets, output_set,intprods,conv_subscripts)
        out_inds, input_sets, idx_removed, idx_contract, new_conv = contract_tuple
        
        out_inds_format=sorted("".join(out_inds))
        out_inds_format="".join(out_inds_format)
        
        # Compute cost, scale, and size
        cost = helpers.flop_count(idx_contract, idx_removed, len(contract_inds), size_dict, conv_subscripts,intprods,
                                  out_inds_format, "","",new_conv)
        cost_list.append(cost)
        scale_list.append(len(idx_contract))
        size_list.append(helpers.compute_size_by_dict(out_inds, size_dict, conv_subscripts,intprods,out_inds_format,new_conv))
    ...
    #path_print = PathInfo(contraction_list, input_subscripts, output_subscript, indices, path, scale_list, naive_cost,
                          #opt_cost, size_list, size_dict,)
    if einoutput:
        path_print = PathInfo(contraction_list, input_subscripts, einoutput, indices, path, scale_list, naive_cost,
                              opt_cost, size_list, size_dict)
    else:
        path_print = PathInfo(contraction_list, input_subscripts, output_subscript, indices, path, scale_list, naive_cost,
                              opt_cost, size_list, size_dict)

...
class PathOptimizer(object):
    ...
    def __call__(self, inputs, output, size_dict, memory_limit=None, conv_subscripts=""):
...
def calc_k12_flops(inputs, output, remaining, i, j, size_dict, conv_subscripts="",intprods={},intermediate=""):
    """
    ...
    conv_subscripts: iterable
        Convolutional indices
    intprods: iterable
        A dictionary of intermediate products
    ...
    new_conv = 0 #Is a new convolution occurring?
    
    k1format= sorted("".join(k1))
    k1format="".join(k1format)
    
    k2format= sorted("".join(k2))
    k2format="".join(k2format) 
    either = k1 | k2
    eitherformat = "".join(either) #Properly formatted for the dictionary.
    eitherformat= "".join(sorted(eitherformat))

    shared = k1 & k2
    keep = frozenset.union(output, *map(inputs.__getitem__, remaining - {i, j})) 
    #Add the base tensors a part of k1  & k2
    if eitherformat in intprods: #Check if this intermediate is already in the dictionary.
        intprods[eitherformat] = intprods[eitherformat]+intprods[k1format]  
        intprods[eitherformat] = intprods[eitherformat]+intprods[k2format]
    else:
        intprods[eitherformat] = intprods[k1format]
        intprods[eitherformat] = intprods[eitherformat]+intprods[k2format] 
    
    intprods[eitherformat] = list(set(intprods[eitherformat])) #Removes situation with duplicates.
    k12 = either & keep
    
    if conv_subscripts:
        for j in conv_subscripts:
            if (j in k1) and (j in k2):
                new_conv = 1 #A new convolution is about to occur. 
    
    cost = helpers.flop_count(either, shared - keep, 2, size_dict, conv_subscripts, intprods, eitherformat)
 
    return k12, cost, new_conv


def _compute_oversize_flops(inputs, remaining, output, size_dict, conv_subscripts="", intprods={}, intermediate=""):
    """
    ...
    return helpers.flop_count(idx_contraction, inner, num_terms, size_dict, conv_subscripts, intprods)


def optimal(inputs, output, size_dict, memory_limit=None,conv_subscripts="", intprods={}):
    """
    ...
    def _optimal_iterate(path, remaining, inputs, flops):
        ...
        for i, j in itertools.combinations(remaining, 2):
            ...
            try:
                k12, flops12, new_conv = result_cache[key]
            except KeyError:
                k12, flops12, new_conv = result_cache[key] = calc_k12_flops(inputs, output, remaining, i, j, 
                                                                            size_dict, conv_subscripts,intprods)
            
            k12format=sorted("".join(k12))
            k12format="".join(k12format)
            ...
            if memory_limit not in _UNLIMITED_MEM:
                try:
                    ...
                    size12 = size_cache[k12] = helpers.compute_size_by_dict(k12, size_dict, conv_subscripts,intprods,k12format,new_conv)

                # possibly terminate this path with an all-terms einsum
                if size12 > memory_limit:
                    new_flops = flops + _compute_oversize_flops(inputs, remaining, output, 
                                                                size_dict, conv_subscripts,intprods,k12format,new_conv)
                    if new_flops < best['flops']:
            ...
            k1format=sorted("".join(inputs[i]))
            k1format="".join(k1format)
            
            k2format=sorted("".join(inputs[j]))
            k2format="".join(k2format)
            
            intprods[k12format]=list(set(intprods[k1format])|set(intprods[k2format]))
            #import pdb; pdb.set_trace()
            #Add k12 to the intermediate dictionary. 
...
class BranchBound(PathOptimizer):
    ...
    def __call__(self, inputs, output, size_dict, memory_limit=None, conv_subscripts=""):
        ...
        size_cache = {k: helpers.compute_size_by_dict(k, size_dict, conv_subscripts) for k in inputs}
        result_cache = {}
        ...
        def _branch_iterate(path, inputs, remaining, flops, size):
            ...
            def _assess_candidate(k1, k2, i, j):
                ...
                try:
                    ...
                    k12, flops12 = result_cache[k1, k2] = calc_k12_flops(inputs, output, remaining, i, j, size_dict, conv_subscripts)

                try:
                    size12 = size_cache[k12]
                except KeyError:
                    size12 = size_cache[k12] = helpers.compute_size_by_dict(k12, size_dict, conv_subscripts)
                ...
                if (memory_limit not in _UNLIMITED_MEM) and (size12 > memory_limit):
                    ...
                    new_flops = flops + _compute_oversize_flops(inputs, remaining, output, size_dict, conv_subscripts)
...
class DynamicProgramming(PathOptimizer):
    ...
    def __call__(self, inputs, output, size_dict, memory_limit=None, conv_subscripts=""):
        ...
        for g in subgraphs:
            ...
            subgraph_contractions_size.append(helpers.compute_size_by_dict(i, size_dict, conv_subscripts))
...
def auto(inputs, output, size_dict, memory_limit=None, conv_subscripts="",intprods={}):
    """Finds the contraction path by automatically choosing the method based on
    how many input arguments there are.
    """
    N = len(inputs)
    return _AUTO_CHOICES.get(N, greedy)(inputs, output, size_dict, memory_limit,conv_subscripts,intprods)
...
def auto_hq(inputs, output, size_dict, memory_limit=None,conv_subscripts="",intprods={},intermediate=""):
    """Finds the contraction path by automatically choosing the method based on
    ...
    N = len(inputs)
    return _AUTO_HQ_CHOICES.get(N, random_greedy_128)(inputs, output, size_dict, memory_limit, conv_subscripts,intprods)

...
Tensor Tensor::CalcPseudoInverse(size_t slice_mode1, size_t slice_mode2,
                                 std::vector<size_t> slice_index) 
{
    ...
    std::vector<size_t> out_tensor_size = tensor_size;
    out_tensor_size[slice_mode1] = tensor_size[slice_mode2];
    out_tensor_size[slice_mode2] = tensor_size[slice_mode1];
    Tensor out(out_tensor_size);

    size_t num_mat_components = tensor_size[slice_mode1] * tensor_size[slice_mode2]; 
    for(size_t i = 0; i < num_mat_components; i++) {
        out.data[i] = *(pseudo_inverse.data() + i);
    }

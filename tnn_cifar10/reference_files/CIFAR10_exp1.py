...
def parse():
    ...
    parser.add_argument("--outputs-path", default = "./outputs", type = str,
...
def main():
    ...
    outputs_path = args.outputs_path
     ...
     # create the name of the current compression rate
    outputs_path = os.path.join(outputs_path, args.decompose_type)
    outputs_path = os.path.join(outputs_path, "cr"+str(args.compression_rate))
    if not os.path.exists(outputs_path):
         raise Exception("model_path not created.")
    ...
    if not os.path.exists(tensorboard_path):
        raise Exception("tensorboard_path not created.")
    ...
    train_samples = len(train_loader) * args.batch_size
    valid_samples = len(valid_loader) * args.batch_size
    ...
    if args.evaluate:
        validate(test_loader, model, tensorboard_writer)
        return
    ...
    for epoch in range(args.start_epoch, args.epoch_num):
        ...
        prec1 = validate(test_loader, model,tensorboard_writer)
...
def save_checkpoint(state, is_best, filename):
    ...
    if is_best:
        new_name = "./outputs/"+str(args.decompose_type)+ "/cr" + str(args.compression_rate) + "/model_best.pth.tar"
